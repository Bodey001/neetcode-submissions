    ...
    def isValidSudoku(self, board: List[List[str]]) -> bool:
        ...
        for i in range(len(board)):
            for j in range(len(board[i])):
                ...
                if element in rows_hash_set[i] and element != ".":
                    return False
                rows_hash_set[i].add(element)

                if element in cols_hash_set[j] and element != ".":
                    return False
                cols_hash_set[j].add(element)

                if element in squares_hash_set[box_index] and element != ".":
                    return False
                squares_hash_set[box_index].add(element)
